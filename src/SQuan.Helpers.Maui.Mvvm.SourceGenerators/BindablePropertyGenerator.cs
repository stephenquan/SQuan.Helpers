...
public class BindablePropertyGenerator : IIncrementalGenerator
{
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		context.RegisterSourceOutput(properties, (spc, propertySymbol) =>
		{
			...
			bool hasInitializer = propertySyntax.Initializer is not null;
			...
			string createDefaultValue = string.Empty;
			string defaultValueCreatorProperty = string.Empty;
			string defaultValueCreatorImplementation = string.Empty;

			if (hasInitializer)
			{
				defaultValueCreatorProperty = $$"""
            defaultValueCreator: (b) => (({{className}})b).__On{{propertyName}}CreateDefaultValue(),
""";
				defaultValueCreatorImplementation = $$"""
	object __On{{propertyName}}CreateDefaultValue()
	{
		__Is{{propertyName}}CreatingDefaultValue = true;
		var result = {{propertyName}};
		__Is{{propertyName}}CreatingDefaultValue = false;
		return result;
	}
""";
			}
...
{{additionalChangingCommands}}
	...
	bool __Is{{propertyName}}CreatingDefaultValue { get; set; } = false;

	{{access}} partial {{typeName}} {{propertyName}}
	{
		{{getModifiers}} get => __Is{{propertyName}}CreatingDefaultValue ? field : ({{typeName}})GetValue({{propertyName}}Property);
